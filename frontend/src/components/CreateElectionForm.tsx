...
export const CreateElectionForm = ({ onClose, onSuccess }: CreateElectionFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!startDate || !startTime || !endDate || !endTime) {
      setError('Data e ora di inizio e fine sono obbligatori');
      return;
    }
    ...
    try {
      ...
      const eligibleStudentsData = Array.from(selectedStudents).map(studentId => ({
        election_id: electionData.id,
      ...
      setError(error instanceof Error ? error.message : 'Errore nella creazione dell\'elezione');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="es. Presidente Consiglio Studentesco"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
              placeholder="Descrivi l'elezione e il ruolo da eleggere..."
            />
          </div>
              ...
              {candidates.map((candidate, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                  />
                  <textarea
                    value={candidate.description}
                    onChange={(e) => updateCandidate(index, 'description', e.target.value)}
                    placeholder="Breve descrizione del candidato"
                    rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg mb-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition text-sm"
                  />
                  {candidates.length > 1 && (
                    <button
                      type="button"
